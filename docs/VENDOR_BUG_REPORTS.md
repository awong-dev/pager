...
Replaces `sequans_support_request.txt`, which blamed missing SNI. That theory was disproved on
the wire; see `BRINGUP_NOTES.md`. Fill in the bracketed identity fields before sending. Send the
first to Sequans (their forum, or via DPTechnics), the second and third to
<https://github.com/QuickSpot/walter-esp-idf/issues>.

Common details for all three:
