...
1. **A page arriving while the pager light-sleeps** (normal build). The whole power design rests
   on the modem holding an event until the next 5 s wake (`PROTOCOL.md` §8.3).
2. Replies from the pager: no keyboard or button is wired on the bench unit.
3. A CA push from the web app, with a right and a wrong CA.
4. `gnsstest` outdoors: which radio route the modem accepts, time to fix, re-attach time.
5. SMS at all: the SIM may not carry it. Then texts from listed and unlisted numbers.
6. `mqtttest ... emptyca`: TLS with an empty CA slot (the firmware avoids the situation anyway).
7. The accelerometer, and every screen that needs input.
