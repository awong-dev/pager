...
// ---------------------------------------------------------------------------
// TEMPORARY hardware bring-up diagnostic (boot test screen). Self-contained:
// does its own WalterModem::begin()/setOpState() rather than depending on
// net_init() (which needs `ident`, not available this early/pre-provisioning)
// or net_bootstrap_attach() (which attaches to LTE -- not needed for a bare
// GNSS fix and would only slow this down). Not the production init path;
// remove or fold into modes.c once this stops being a bring-up test.
// ---------------------------------------------------------------------------

static volatile bool s_gnss_fix_received = false;
static WMGNSSFixEvent s_gnss_fix = {};

static void pager_gnss_event_handler(WMGNSSEventType type, const WMGNSSEventData *data, void *args)
{
    (void) args;
    if (type == WALTER_MODEM_GNSS_EVENT_FIX) {
        memcpy(&s_gnss_fix, &data->gnssfix, sizeof(WMGNSSFixEvent));
        s_gnss_fix_received = true;
    }
}

extern "C" bool net_boot_diagnostics(double *lat, double *lon, double *confidence,
                                     uint8_t *sat_count, int *batt_mv, int gnss_timeout_s)
{
    if (!WalterModem::begin(PAGER_MODEM_UART)) {
        ESP_LOGI(TAG, "boot diagnostics: WalterModem::begin() failed");
        return false;
    }
    // Default opstate after begin() is already MINIMUM -- no explicit
    // setOpState() call needed here (matches the vendor positioning
    // example, which never sets one before gnssConfig()).

    if (batt_mv) {
        WalterModem::configVoltageMonitor(WALTER_MODEM_VOLTAGE_MONITOR_MODE_ACTIVE, 30, 30);
        WalterModemRsp rsp = {};
        if (WalterModem::getVoltage(&rsp)) {
            // Hardware bring-up finding (docs/DEVICE_TASKS_LOG.md): the
            // *100 "tenths of a volt" conversion this codebase assumed
            // elsewhere (net_get_battery_mv()) produced an implausible
            // 519.7V on this board; the raw field (5197) is a plausible
            // 5.197V for a USB-powered board with no battery attached, so
            // the raw value is used directly as millivolts here instead.
            // net_get_battery_mv() itself is untouched pending a battery
            // being connected to confirm this against a known-good 3-4.2V
            // LiFePO4 reading rather than just USB-rail plausibility.
            ESP_LOGI(TAG, "boot diagnostics: raw voltage fields mode=%d status=%d threshold=%u "
                          "period=%u voltage=%u",
                     (int) rsp.data.voltage.mode, (int) rsp.data.voltage.status,
                     (unsigned) rsp.data.voltage.threshold, (unsigned) rsp.data.voltage.period,
                     (unsigned) rsp.data.voltage.voltage);
            *batt_mv = (int) rsp.data.voltage.voltage; // raw field used directly as mV (UNVERIFIED)
        } else {
            ESP_LOGI(TAG, "boot diagnostics: getVoltage() failed");
            *batt_mv = 0;
        }
    }

    // No LTE attach: no SIM card in this bring-up session, so an attach
    // would just spin for its full timeout with no chance of succeeding.
    // gnssPerformAction() was confirmed to fail outright (fast, not a
    // timeout) without a valid GNSS clock -- the vendor positioning example
    // normally gets this from LTE/NITZ. GPS itself doesn't need network
    // time (satellites broadcast precise UTC once acquired); a rough
    // manually-seeded clock is only there to narrow the almanac/ephemeris
    // search window, so seed one directly rather than requiring a SIM.
    WalterModem::gnssSetUTCTime((uint64_t) 1789646400); // ~2026-09-17, rough seed only
    WalterModem::setGNSSEventHandler(pager_gnss_event_handler, nullptr);
    if (!WalterModem::gnssConfig()) {
        ESP_LOGI(TAG, "boot diagnostics: gnssConfig() failed");
        return false;
    }
    s_gnss_fix_received = false;
    if (!WalterModem::gnssPerformAction()) {
        ESP_LOGI(TAG, "boot diagnostics: gnssPerformAction() failed");
        return false;
    }
    ESP_LOGI(TAG, "boot diagnostics: GNSS fix requested, waiting up to %ds", gnss_timeout_s);

    int64_t start = esp_timer_get_time();
    while (!s_gnss_fix_received) {
        if ((esp_timer_get_time() - start) > (int64_t) gnss_timeout_s * 1000000) {
            ESP_LOGI(TAG, "boot diagnostics: GNSS fix timed out after %ds", gnss_timeout_s);
            return false;
        }
    ...
    if (lat) {
        *lat = s_gnss_fix.latitude;
    }
    if (lon) {
        *lon = s_gnss_fix.longitude;
    }
    if (confidence) {
        *confidence = s_gnss_fix.estimatedConfidence;
    }
    if (sat_count) {
        *sat_count = s_gnss_fix.satCount;
    }
    ESP_LOGI(TAG, "boot diagnostics: GNSS fix lat=%.6f lon=%.6f confidence=%.1f sats=%u",
             s_gnss_fix.latitude, s_gnss_fix.longitude, s_gnss_fix.estimatedConfidence,
             s_gnss_fix.satCount);
    return true;
}
